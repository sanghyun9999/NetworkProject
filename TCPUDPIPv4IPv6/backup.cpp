...
//	if (tcp_sock == INVALID_SOCKET || udp_sock == INVALID_SOCKET
//		|| tcp6_sock == INVALID_SOCKET || udp6_sock == INVALID_SOCKET) err_quit("socket()");
//
//	if (setsockopt(tcp6_sock, IPPROTO_IPV6, IPV6_V6ONLY, (char*)&opt, sizeof(opt)) == SOCKET_ERROR)
//		err_quit("setsockopt tcp6");
//	if (setsockopt(udp6_sock, IPPROTO_IPV6, IPV6_V6ONLY, (char*)&opt, sizeof(opt)) == SOCKET_ERROR)
//		err_quit("setsockopt udp6");
//
//	// IPv4 구조체
...
//
//	// tcp6 bind
//	retval = bind(tcp6_sock, (struct sockaddr*)&serveraddr6, sizeof(serveraddr6));
//	if (retval == SOCKET_ERROR) err_quit("bind() - TCP6");
//
//	// tcp6 listen
//	retval = listen(tcp6_sock, SOMAXCONN);
//	if (retval == SOCKET_ERROR) err_quit("listen()");
//
//	// udp6 bind
//	retval = bind(udp6_sock, (struct sockaddr*)&serveraddr6, sizeof(serveraddr6));
//	if (retval == SOCKET_ERROR) err_quit("bind() - UDP6");
//
//	printf("서버실행디버그\n");
//
...
//			// [UDP 처리 구역
//			// 이곳에 recvfrom()을 사용하여 UDP 데이터를 받고 처리하는 코드를 작성.
//
//			// 데이터 통신에 사용할 변수
...
//			// [TCP 연결 수락 구역] 
//			// 이곳에 accept()를 호출하고, AddSocketInfo()를 사용하여 새 클라이언트를 등록하는 코드를 작성.
//
//			// 데이터 통신에 사용할 변수
//			SOCKET client_sock;
//			struct sockaddr_in clientaddr;
...
//				// [TCP 데이터 수신 구역]
//				// 이곳에 recv()를 사용하여 데이터를 받고, 
//				// 만약 연결이 끊겼다면 RemoveSocketInfo()를 호출하는 코드를 작성.
//
//				if (FD_ISSET(ptr->sock, &copy_reads)) {
//					// 데이터 받기
